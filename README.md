...
- **自動起動・自動停止**: `llama-server` の `base_url` に既に応答があればそれ（人が起動したもの）をそのまま使い、止めない。応答が無く `launch` があれば自動で起動し、`startup_seconds`（既定180秒）まで起動を待つ。生成が終われば自分が起動したサーバーだけを自動で停止する。前回異常終了して残ったサーバーがあれば、次にリースを取った側が自動で終了させる
