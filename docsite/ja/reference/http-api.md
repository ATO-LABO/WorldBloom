...
- `python viewer/server.py --runs <runs>` だけで起動すると、`/api/...` の**書き込み系エンドポイントは使えません**（`job_store` が無いため `503 unavailable` になります）。読み取り専用の `GET /api/status/local` だけは常に使えます。
...
| POST | `/api/configs/<id>/duplicate` | 実行設定を複製 |
...
`--control` があれば `job_store` の有無に関係なく動きます（世界・ジャンルの編集はGA実験と独立のため）。

| メソッド | パス | 用途 |
|---|---|---|
| GET / POST | `/api/worlds` | 世界の一覧取得 / 新規作成 |
...
| GET / POST | `/api/genres` | ジャンルの一覧取得 / 新規作成 |
...
| メソッド | パス | 用途 |
|---|---|---|
| POST | `/exp/<experiment>/selection` | 採用・保留・除外の選定状態を更新（`job_store` が無くても動く読み取り専用ビューア以外の経路。実行中の実験には `assert_run_idle` で拒否） |
...
これ以外にも `viewer/run_catalog.py`（`/api/runs`・`/api/selected`）・`viewer/workbench_pages.py`（`/api/configs/<id>/duplicate` 以外は主に画面表示用のGET）がありますが、いずれも画面のUI操作に対応する内部APIです。正確な一覧は各ファイルの `dispatch()`/`_resolve()` を参照してください。
