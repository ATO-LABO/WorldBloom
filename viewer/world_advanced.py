...
def render(world, store, jobs):
    ...
    body+='<footer class="ux-footer"><p role="status" data-advanced-status>'+('保存された設定を表示しています' if jobs else '閲覧専用です')+'</p><div'+('' if jobs else ' hidden')+'><button type="button" data-advanced-reset>変更を戻す</button> <button type="button" data-advanced-check>設定を確認</button> <button type="button" data-advanced-save class="ux-primary">変更を保存</button></div></footer></div>'
